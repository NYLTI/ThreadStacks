...
public class ThreadHandler {
  ...
  public Mono<ServerResponse> createThread(ServerRequest request) {
    ...
    return request.bodyToMono(Thread.class).flatMap(validationUtil::validate).map(thread -> {
      ...
      return thread;
    }).flatMap(threadRepository::save).flatMap(
        savedThread -> ServerResponse.created(URI.create("/threads/" + savedThread.getId()))
  ...
  public Mono<ServerResponse> getThreadById(ServerRequest request) {
    ...
    return threadRepository.findById(threadId)
        .flatMap(
            thread -> ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).bodyValue(thread))
  ...
  public Mono<ServerResponse> deleteThread(ServerRequest request) {
    ...
    if (threadId == null) {
      return ServerResponse.status(HttpStatus.BAD_REQUEST)
          .bodyValue("Missing threadId pathVariable");
    }
    return threadRepository.findById(threadId)
        .switchIfEmpty(Mono.error(new ResourceNotFoundException("Thread not found")))
        .flatMap(thread -> {
          boolean isModerator =
              userRoles.stream().anyMatch(role -> role.equalsIgnoreCase(thread.getRoomName()));
          if (!userName.equals(thread.getUserName()) && !isModerator) {
            return ServerResponse.status(HttpStatus.FORBIDDEN)
                .bodyValue("You are not authorized to delete this thread.");
          }
          thread.setContent("[deleted]");
          thread.setUserName(null);
          return threadRepository.save(thread)
              .then(ServerResponse.ok().bodyValue("Thread deleted"));
        });
  }
